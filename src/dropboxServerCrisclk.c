#define dprint1(x) do { if (CRISCLK_DEBUG >= 1) fprintf x; } while (0)
#define dprint2(x) do { if (CRISCLK_DEBUG >= 2) fprintf x; } while (0)
...
void cl_list_add(crisclk_sv_t *clk, int clskt) {
	...
	if(CRISCLK_DEBUG) cl_list_print(clk);
...
void cl_list_remove_one(crisclk_sv_t *clk, crisclk_cl_t *clconn) {
	...
	fprintf(stderr, "Error: cl_list_remove_one() haven't found the client connection in list.\n");
}

void sv_close_connection(crisclk_sv_t *clk, crisclk_cl_t *clconn, time_t code) {

	dprint1((stderr, "[Cl %d] Sending connection to garbage...\n", clconn->sockfd));
	time(&clconn->tmeout);
	char *data = (char *)&code;
...
crisclk_cl_t *cl_list_tail(crisclk_sv_t *clk) {
	...
	dprint2((stderr, "[Cl %d] List tail found: %p\n", iterator->sockfd, iterator));
...
void *crisclk_sv_deamon(void *arg) {
	...
	while(true) {
		if((newsockfd = accept(clk->sockfd, (struct sockaddr *)&cli_addr, &clilen)) == -1) {
		...
		pthread_create(&cl_list_tail(clk)->thread, NULL, crisclk_sv_client_thread, clk);
		pthread_mutex_unlock(&clk->access);
	}
}
...
void *crisclk_sv_toutth(void *arg) {
	...
	while(true){
		...
		/**
		 * WARNING: the time_t arithmetic is guaranteed to work only with POSIX systems.
		 * time_t isn't necessarilly a number in seconds.
		 */
		...
		int i = 0;
			...
			if(iterator->garbag && difftime(st, iterator->tmeout) > CRISCLK_GARBAGE_TME) {
...
int crisclk_sv_deamon_run(crisclk_sv_t *clk) {
	...
	if((ret = setsockopt(clk->sockfd, SOL_SOCKET, SO_REUSEADDR, &(int){1}, sizeof(int))) < 0) {
		perror("setsockopt(SO_REUSEADDR) failed.\n");
		close(clk->sockfd);
		return ret;
	}
	if((ret = bind(clk->sockfd, (struct sockaddr *)&clk->svaddr, sizeof(clk->svaddr))) < 0) {
		perror("ERROR on binding.\n");
		close(clk->sockfd);
		return ret;
	}
	pthread_mutex_init(&clk->access, NULL);
	if((ret = listen(clk->sockfd, clk->bcklog)) != 0) return ret;
	if((ret = pthread_create(&clk->thread, NULL, crisclk_sv_deamon, clk)) != 0) return ret;
	if((ret = pthread_create(&clk->toutth, NULL, crisclk_sv_toutth, clk)) != 0) return ret;
	return ret;
}