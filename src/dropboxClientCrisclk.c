...
void crisclk_cl_connect(crisclk_t *clk) {
	...
	if(!clk->connected) {
		...
		if (connect(clk->sockfd, (struct sockaddr *)&clk->serv_addr, sizeof(clk->serv_addr)) < 0) {
...
time_t crisclk_cl_get_time(crisclk_t *clk) {
	...
	if(!persistent) crisclk_cl_connect(clk);
	...
	if(ts == CRISCLK_DISCONNECT_CODE) {
		fprintf(stderr, "WARNING: Cristian Clock server closed connection (server shutdown).\n");
		crisclk_cl_disconnect(clk);
		return ts;
	} else if(ts == CRISCLK_TIME_OUT_CODE) {
		fprintf(stderr, "WARNING: Cristian Clock server closed connection (timeout).\n");
		return ts;
	}

	dprint1((stderr, "tf = ts+(t1-t0)/2\ntf = %ld+(%ld-%ld)/2\ntf = %ld\n", ts, t1, t0, tf));

	if(!persistent) crisclk_cl_disconnect(clk);
